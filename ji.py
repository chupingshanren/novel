...
def setup(i,start,end):
    ...
    with open('万界圆梦师.html', "a+",encoding='utf-8') as f:
        for i in range(start,end+1):
            f.write('<p><a href="万界圆梦师/'+str(ids[i])+'.html" id="'+str(ids[i])+'">'+lts[i]+'</a></p>\n')
        #f.write('</body></html>')
...
def main(i):
    ...
    tid = i[2]
    headers = {
    'cookie': 'newstatisticUUID=1550838161_368556521; _csrfToken=jYiwmLcaMChAB3NSM1JQFI0oY5EvwJNW36O4TU4w; qdrs=0^%^7C3^%^7C0^%^7C0^%^7C1; showSectionCommentGuide=1; qdgd=1; e1=^%^7B^%^22pid^%^22^%^3A^%^22qd_P_Searchresult^%^22^%^2C^%^22eid^%^22^%^3A^%^22qd_S05^%^22^%^2C^%^22l1^%^22^%^3A3^%^7D; e2=^%^7B^%^22pid^%^22^%^3A^%^22qd_P_Searchresult^%^22^%^2C^%^22eid^%^22^%^3A^%^22^%^22^%^2C^%^22l1^%^22^%^3A2^%^7D; pageOps=1; lrbc=1016314237^%^7C490847239^%^7C0^%^2C1004986370^%^7C352716776^%^7C0^%^2C1011526746^%^7C433975751^%^7C1; rcr=1016314237^%^2C1004986370^%^2C1017010819^%^2C1017281778^%^2C1011526746; bc=1016314237',
    'accept-encoding': 'gzip, deflate, br',
    'accept-language': 'zh-CN,zh;q=0.9',
    'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/67.0.3396.99 Safari/537.36',
    'accept': 'application/json, text/javascript, */*; q=0.01',
    'referer': 'https://read.qidian.com/chapter/F579y05oe8_mkXioLmMPXw2/4WI6VAu7GHb4p8iEw--PPw2',
    'authority': 'read.qidian.com',
    'x-requested-with': 'XMLHttpRequest',
    }  
 
    time.sleep(5+random.randint(10,30))
    chap = requests.get('https://vipreader.qidian.com/ajax/chapterReview/reviewSummary?_csrfToken=jYiwmLcaMChAB3NSM1JQFI0oY5EvwJNW36O4TU4w&bookId='+bookId+'&chapterId='+chapterId, headers=headers).json()
    ...
    # segid = []
    # segids = []
    shutil.copy("comments.html",'万界圆梦师/'+str(tid)+'说.html')
    for i in range(chap["data"]["total"]):
        flag = 1
        num = chap["data"]["list"][i]["reviewNum"]//20+1
        with open('万界圆梦师/'+str(tid)+'说.html', "a+",encoding='utf-8') as f:
    ...
    shutil.copy("comments.html",'万界圆梦师/'+str(tid)+'.html')
    with open('万界圆梦师/'+str(tid)+'.html', "a+",encoding='utf-8') as f:
        if segid.get(-1):
        ...
        for i in range(len(ct)):
            if segid.get(i+1):
                ...
                f.write('<p>'+ct[i]+'</p>\n')
        f.write('<div style="text-align:center"><a href="'+str(tid-1)+'.html">上一章</a><a href="../万界圆梦师.html#'+str(tid)+'">章节目录</a><a href="'+str(tid+1)+'.html">下一章</a>\n')	
        f.write('</body></html>')           
...
if __name__ == '__main__':
    ...
    url = ['https://book.qidian.com/info/1016314237#Catalog','https://www.biqubu.com/book_24888/']
    setup(url,args.start,args.end)
    #    start = 101,end = 120
    # cd C:\Users\22129\mytool\git\novel
    #python ji.py -s 634 -e 648
